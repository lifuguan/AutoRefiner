...
def _load_img_as_tensor(img_path, image_size):
    # 如果img_path开头为s3://的，则使用megfile的读写方法
    if isinstance(img_path, str) and img_path.startswith("s3://"):
...
def load_video_frames(
    ...
    elif isinstance(video_path, str) and os.path.isdir(video_path):
        jpg_folder = video_path
    else:
    ...
    if not video_path.startswith("s3://"):
        ...
        if "scannetpp" in jpg_folder:
            frame_names = [
                p for p in megfile.s3_listdir(jpg_folder)
                if p.startswith("frame") and os.path.splitext(p)[-1].lower() in ['.jpg', '.jpeg', '.png']
            ]
        else:
            frame_names = [
    ...
    print(f"Loaded {num_frames} frames from {jpg_folder}")
