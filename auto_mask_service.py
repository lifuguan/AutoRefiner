...
def search_new_obj(masks_from_prev, mask_list,other_masks_list=None,mask_ratio_thresh=0.05,ratio=0.5, area_threash = 5000):
    new_mask_list = []
...
class AutoMaskService(ServeEngine):
    def __init__(self, name: str, model_config: ModelConfig, engine_config: CustomEngineConfig):
        ...
        self.sam2_checkpoint = os.path.join(model_path, custom_args.get("sam2_checkpoint", "sam2_hiera_large.pt"))
        self.model_cfg = "sam2_hiera_l.yaml"
        self.predictor = build_sam2_video_predictor("sam2_hiera_l.yaml", self.sam2_checkpoint)
        
        # Load SAM1 model
        self.sam_ckpt_path = os.path.join(model_path, custom_args.get("sam_ckpt_path", "sam_vit_l_0b3195.pth"))
        ...
        try:
            ...
            gpu_name = ""
        # 支持高并发的显卡型号关键字
        high_concurrent_gpus = ["L40", "H200", "H100", "H800", "A100", "A800"]
        max_concurrent = 1
        for g in high_concurrent_gpus:
            if g.lower() in gpu_name.lower():
                max_concurrent = 2
                break
    ...
    def process_video(self, request: MaskRequest) -> dict:
        ...
        if True:
            ...
            # Get frame names
            if "scannetpp" in request.image_dir:
                frame_names = [
                    p for p in megfile.s3_listdir(request.image_dir)
                    if p.startswith("frame") and os.path.splitext(p)[-1].lower() in ['.jpg', '.jpeg', '.png']
                ]
            else:
                frame_names = [
                    p for p in megfile.s3_listdir(request.image_dir)
    ...
    def _save_results(self, video_segments, height, width, duration, output_path, video_output_path):
        ...
        self._logger.critical(f"JSON标注已保存至: {output_path}")
        ...
        self._logger.critical(f"掩码视频已保存至: {video_output_path}")
        # 清空占用运行内存
        import gc
        gc.collect()
