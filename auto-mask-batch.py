...
if __name__ == '__main__':
    ...
    # 读取第一张图片获取尺寸信息
    first_image = cv2.imread(os.path.join(args.video_path, frame_names[0]))
    if first_image is None:
        raise ValueError(f"无法读取图片: {os.path.join(args.video_path, frame_names[0])}")
    height, width = first_image.shape[:2]

    image_dir_name = os.path.basename(os.path.normpath(args.video_path))
    ...
    # 生成仅掩码的视频
    mask_only_video_path = os.path.join(base_dir, f"{image_dir_name}.avi")
    # 使用FFV1编码实现无损压缩
